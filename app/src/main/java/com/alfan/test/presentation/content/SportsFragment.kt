...
import com.alfan.test.databinding.FragmentSportsBinding
...
class SportsFragment : DataBindingFragment<FragmentSportsBinding>() {

    private val viewModel by inject<ContentViewModel>()
    private lateinit var contentAdapter: ContentAdapter
    override fun layoutId(): Int = R.layout.fragment_sports
    ...
    private fun getData() {
        viewModel.getTopHeadlinesNews("sports").observe(viewLifecycleOwner) {
            if (it != null) {
                when (it) {
                    is Resource.Loading -> {
                        showLoading(false)
