...
import com.alfan.test.databinding.FragmentGeneralragmentBinding
...
class Generalragment : DataBindingFragment<FragmentGeneralragmentBinding>() {
    private val viewModel by inject<ContentViewModel>()
    private lateinit var contentAdapter: ContentAdapter


    override fun layoutId(): Int = R.layout.fragment_generalragment
    ...
    private fun getData() {
        viewModel.getTopHeadlinesNews("general").observe(viewLifecycleOwner) {
            if (it != null) {
                when (it) {
                    is Resource.Loading -> {
                        showLoading(false)
