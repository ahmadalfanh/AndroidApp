...
import com.alfan.test.databinding.FragmentScienceBinding
...
class ScienceFragment : DataBindingFragment<FragmentScienceBinding>() {
    private val viewModel by inject<ContentViewModel>()
    private lateinit var contentAdapter: ContentAdapter
    override fun layoutId(): Int = R.layout.fragment_science
    ...
    private fun getData() {
        viewModel.getTopHeadlinesNews("science").observe(viewLifecycleOwner) {
            if (it != null) {
                when (it) {
                    is Resource.Loading -> {
                        showLoading(false)
