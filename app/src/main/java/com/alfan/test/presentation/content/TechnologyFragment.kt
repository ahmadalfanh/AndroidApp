...
import com.alfan.test.databinding.FragmentTechnologyBinding
...
class TechnologyFragment : DataBindingFragment<FragmentTechnologyBinding>() {
    private val viewModel by inject<ContentViewModel>()
    private lateinit var contentAdapter: ContentAdapter
    override fun layoutId(): Int = R.layout.fragment_technology
    ...
    private fun getData() {
        viewModel.getTopHeadlinesNews("technology").observe(viewLifecycleOwner) {
            if (it != null) {
                when (it) {
                    is Resource.Loading -> {
                        showLoading(false)
