...
class HealthFragment : DataBindingFragment<FragmentHealthBinding>() {
    ...
    private fun getData() {
        viewModel.getTopHeadlinesNews("health").observe(viewLifecycleOwner) {
            if (it != null) {
                when (it) {
                    is Resource.Loading -> {
                        showLoading(false)
