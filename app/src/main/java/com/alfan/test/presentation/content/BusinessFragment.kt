...
import com.alfan.test.databinding.FragmentBusinessBinding
...
class BusinessFragment : DataBindingFragment<FragmentBusinessBinding>() {
    private val viewModel by inject<ContentViewModel>()
    private lateinit var contentAdapter: ContentAdapter


    override fun layoutId(): Int = R.layout.fragment_business
    ...
    private fun getData() {
        viewModel.getTopHeadlinesNews("business").observe(viewLifecycleOwner) {
            if (it != null) {
                when (it) {
                    is Resource.Loading -> {
                        showLoading(false)
